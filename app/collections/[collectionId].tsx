...
import { filterInventoryItems, getCollectionInventoryOverview, getInventorySetFilterOptions } from "@/services/inventory-query";
...
export default function CollectionDetailScreen() {
  ...
  const collection = useMemo(
    () => (collectionsQuery.data ?? []).find((c) => c.collectionId === collectionId) ?? null,
  ...
  const inventoryQuery = useQuery({
    queryKey: ["inventory-overview", collectionId],
    queryFn: () => {
      if (!collectionId) {
        throw new Error("No se recibio el id de la coleccion.");
      }

      return getCollectionInventoryOverview(collectionId);
    },
  ...
  const errorMessage =
    inventoryQuery.error instanceof Error
      ...
      : "No se pudo cargar el inventario de la coleccion.";

  if (!collectionId) {
    return (
      <SafeAreaView className="flex-1 bg-mist">
        <View className="flex-1 items-center justify-center p-6">
          <Text className="text-sm text-red-700">No se pudo identificar la coleccion.</Text>
          <Pressable onPress={() => router.back()} className="mt-4 rounded-xl bg-ink px-4 py-2">
            <Text className="text-sm font-semibold text-mist">Volver</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    );
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        <View className="w-full self-center max-w-3xl gap-4">
          <View className="rounded-3xl bg-ink p-6">
            <Text className="text-3xl font-bold text-mist">{collection?.name ?? "Coleccion"}</Text>
            <Text className="mt-2 text-sm leading-5 text-slate-300">
              Tus cartas en esta coleccion con cantidad, estado y variacion de precio.
            </Text>
            <View className="mt-4 flex-row flex-wrap gap-3">
              <Pressable
                onPress={() => router.push(`/add-card?collectionId=${collectionId}`)}
          ...
          {inventoryQuery.data && inventoryQuery.data.items.length === 0 ? (
            <View className="rounded-2xl border border-slate-200 bg-white p-6">
              <Text className="text-base font-semibold text-ink">Esta coleccion esta vacia.</Text>
              <Text className="mt-2 text-sm leading-5 text-slate-700">
                Agrega tu primera carta o explora el catalogo para ver las disponibles.
              </Text>
              <Pressable
                onPress={() => router.push(`/add-card?collectionId=${collectionId}`)}
          ...
          {inventoryQuery.data && inventoryQuery.data.items.length > 0 ? (
            <>
              ...
              <View className="rounded-2xl border border-slate-200 bg-white p-4">
                <Text className="text-xs uppercase tracking-wide text-slate-500">Totales de la coleccion</Text>
                <Text className="mt-2 text-sm text-slate-700">
                  Cartas: {inventoryQuery.data.totalCardsCount} · Valor:{" "}
                  {formatUsd(inventoryQuery.data.totalCollectionValueUsd)}
                </Text>
              </View>
                    ...
                    >
                      ...
                      <View className="mt-4 gap-1">
                        ...
                        <Text className="text-sm text-slate-700">
                          Precio actual: {formatUsd(item.currentPriceUsd)}
                        </Text>
